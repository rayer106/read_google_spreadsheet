...
export const buyDataParse = async (map) => {
    // 遍历 Map，category是工作表名称，items是工作表中的数据
    for (const [category, items] of map) {
        console.log(`Category: ${category}`);
        for(let item of items) {
            console.log(`  ${item.link}`);
            // let domesticLink = getDomesticUrls(item.link);
            // if(!domesticLink) {
...
async function getDataFromLink(rawUri) {
    if(!rawUri) {
    ...
    if(rawUri.includes('weidian.com')) {
        return await getDataFromWeidian(rawUri);
    } else if(rawUri.includes('taobao.com')) {
        return await getDataFromTaobao(rawUri);
    } else if(rawUri.includes('1688.com')) {
        return await getDataFromAli1688(rawUri);
    } else {
        console.log('不支持的链接');
        return null;
    }
...
export async function getDataFromAli1688(rawUri) {
    try {
...
export async function getDataFromTaobao(rawUri) {
    try {
        const response = await axios.get(rawUri);
        //const $ = cheerio.load(response.data);
...
export async function getDataFromWeidian(rawUri) {
    try {
        const response = await axios.get(rawUri);
        const $ = cheerio.load(response.data);
        const data_obj = JSON.parse($('#__rocker-render-inject__').attr('data-obj'));
